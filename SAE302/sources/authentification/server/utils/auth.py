    ...
    if len(client_connected) > 1:
        for c in client_connected:
            if c == client_id:
                ...
                time.sleep(0.5)
                if os.path.exists(f"{os.getcwd()}/servers.json"):
                    with open(f"{os.getcwd()}/servers.json", "r") as file:
    ...
    try:
        ...
        if client_type == "client":
            ...
            if client_username in client_connected:
                ...
                return None, None
            if os.path.exists(f"{os.getcwd()}/users.txt"):
                with open(f"{os.getcwd()}/users.txt", "r") as file:
